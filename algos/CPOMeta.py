    ...
    def train_CPOMeta(self, writer, save_info_obj):
        ...
        for i_iter in range(self.args.update_iter_num, self.args.max_iter_num):
            ...
            for local_iter in range(self.args.env_num):
                t1 = time.time()
                memory, _ = self.collect_samples(self.envs[local_iter], self.meta_policy, seed=self.args.seed)
                t2 = time.time()
                sample_time += t2 - t1
                ...
                for k_iter in range(self.args.local_num):
                    ...
                    t1 = time.time()
                    local_memory, _ = self.collect_samples(self.envs[local_iter], self.local_policy, seed=local_iter)
