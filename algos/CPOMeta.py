...
class CPOMeta:
    ...
    def meta_line_search(self, x, batch, fullstep, max_backtracks = 20, accept_ratio=0.1):
        ...
        #expected_reward_improve_full = -g.dot(fullstep)
        #expected_cost_improve_full = -a.dot(fullstep)
    ...
    def collect_samples(self, env, policy, seed):
        ...
        for worker_memory in worker_memories:
            ...
            #print('worker memory state shape: ',np.stack(worker_memory.sample().state).shape)
        #batch = memory.sample()
    ...
    def update(self, batch, meta_update=False, local_update=False):
        ...
        try:
            ...
            # Recover step
            cpo_success = False
            print('a mean: ', abs(a).mean())
            step, = self.trpo_problem(a, max_kl)
            #print('rec1over mean: ', abs(step.detach()).mean())
        ...
        if meta_update or local_update:
            ...
            for i in range(self.param_size):
                if cpo_success:
                    ...
                    if meta_update:
                        a_grad = torch.autograd.grad(loss_cost_grad[i], self.meta_policy.parameters(), retain_graph=True)
                    elif local_update:
                        a_grad = torch.autograd.grad(loss_cost_grad[i], self.local_policy.parameters(), retain_graph=True)
    ...
    def project_step(self, batch, meta_step):
        ...
        try:
            ...
            #print('before projection: ', torch.norm(step))
            step = step / torch.norm(step)
        except:
    ...
    def train_CPOMeta(self, writer, save_info_obj):
        ...
        print(self.args.update_iter_num)
        print(self.args.max_iter_num)
        for i_iter in range(self.args.update_iter_num, self.args.max_iter_num):
            ...
            for batch_iter in range(K):
                ...
                v_loss_meta += reward_v_loss
                c_loss_meta += cost_v_loss
                reward_loss_meta += reward_loss
    
                m_env_avg_reward += log['env_avg_reward']
                m_rewards_std += log['std_reward']
                m_env_avg_cost += log['env_avg_cost']
            ...
            projected_step = self.project_step(meta_memory.sample(), meta_update_sum/K)

            if abs(projected_step).mean() != 0:
                success, _, projected_step = self.meta_line_search(prev_params, meta_memory.sample(), projected_step)
                print(success, 'meta step mean: ', abs(projected_step).mean())

            new_params = prev_params + projected_step * self.args.learning_rate
            ...
            # calculate values
            print('eval shape: ', np.stack(eval_memory.sample().cost).shape)
            costs =  torch.from_numpy(np.stack(eval_memory.sample().cost)[:self.args.max_batch_size]).to(self.dtype).to(self.device) #[:args.batch_size]
            masks = torch.from_numpy(np.stack(eval_memory.sample().mask)[:self.args.max_batch_size]).to(self.dtype).to(self.device)
            eval_cost = estimate_constraint_value(costs, masks, self.args.gamma, self.device)[0].to(torch.device('cpu'))
            ...
            # save the best model
            # and eval_log['std_reward'] <= best_std
            if eval_log['env_avg_reward'] >= best_avg_reward:
