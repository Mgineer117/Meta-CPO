...
def generate_number(min_value, max_value, seed=5, is_int=False):
    random.seed(seed)
...
class StcircleLevel2(BaseTask):
    ...
    def __init__(self, config) -> None:
        ...
        radius = generate_number(1.5, 2.0)
        scale = generate_number(0.55, 0.65)
        distance = round(radius*scale, 3)
        num_walls = 2
        num_hazards = generate_number(10, 12, is_int=True)
        print('radius: ', radius)
        ...
        self._add_geoms(Hazards(num=num_hazards, size=0.12, cost=3, placements=[(-1.8, -1.8, 1.8, 1.8)],is_constrained=True))
