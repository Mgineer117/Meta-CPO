...
class StbuttonLevel1(BaseTask):
    """An agent must press a goal button."""

    def __init__(self, config) -> None:
        super().__init__(config=config)
        placement = generate_number(1.75, 2.0)

        num_buttons = generate_number(3, 6, is_int=True) # 3 - 6  (test 6 - 10)
        ...
        self._add_geoms(Buttons(num=num_buttons, keepout=0.2))
