...
from safety_gymnasium.assets.geoms import Buttons, Goal, Hazards, Sigwalls
from safety_gymnasium.bases.base_task import BaseTask

import random
def generate_number(min_value, max_value, seed=9, is_int=False):
    random.seed(seed)
...
class StbuttonLevel2(BaseTask):
    """An agent must press a goal button."""

    def __init__(self, config) -> None:
        super().__init__(config=config)

        placement = generate_number(2.6, 2.9)

        num_buttons = generate_number(6, 10, is_int=True) # 2 - 8  (test 12)
        num_hazards =  num_buttons + 6 # + 6 was chosen to invoke difficult env setting

        self.placements_conf.extents = [-placement, -placement, placement, placement]


        print('num button: ', num_buttons)
        print('placement size: ', placement)

        self._add_geoms(Buttons(num=num_buttons, keepout=0.2, is_constrained = True))
        self._add_geoms(Goal(size=self.buttons.size * 2, alpha=0.1))  # pylint: disable=no-member
        self._add_geoms(Hazards(num=num_hazards, keepout=0.2))
